---
layout: single
title: "Curriculum Vitae"
permalink: /cv/
---

<!-- # Curriculum Vitae -->

Below you can view my CV directly.  
You can also download it using the button below.

---

## 📄 View my CV

<div style="width:100%; height:85vh; border: 1px solid #ccc;">
  <iframe 
      src="{{ site.baseurl }}/assets/cv/CV_en.pdf" 
      style="width:100%; height:100%;" 
      frameborder="0">
  </iframe>
</div>

---

## ⬇️ Download my CV

<p>
  <a href="{{ site.baseurl }}/assets/cv/CV_en.pdf" download class="btn btn--primary">
    Download CV — English (PDF)
  </a>
  <a href="{{ site.baseurl }}/assets/cv/CV_fr.pdf" download class="btn btn--info">
    Télécharger le CV — Français (PDF)
  </a>
</p>
